...
#endif
#define danghui_top(rL) *(DWORD*)(rL + 16)
...
unsigned int r_lua_newthread(unsigned int rL)
{
	...
	if (!danghui_unprotectednewthread)
		danghui_unprotectednewthread = Retcheck::unprotect<DWORD>((BYTE*)(danghui_rbase() + DH_NEWTHRE));
...
void r_luaV_gettable(unsigned int rL, r_TValue* t, r_TValue* key, r_TValue* val)
{
	...
	_gettable gettable = (_gettable)(danghui_rbase() + DH_VGETTAB);
...
void r_luaV_settable(unsigned int rL, r_TValue* t, r_TValue* key, r_TValue* val)
{
	...
	_settable settable = (_settable)(danghui_rbase() + DH_VSETTAB);
...
void* r_luaF_newCclosure(unsigned int rL, int nelems, void* e)
{
	...
	_newCclosure newCclosure = (_newCclosure)(danghui_rbase() + DH_NEWCCLO);
...
int r_lua_xcall(unsigned int rL, int nargs, int nresults, int* noresults)
{
	...
	if (!danghui_unprotectedxcall)
		danghui_unprotectedxcall = Retcheck::unprotect<DWORD>((BYTE*)(danghui_rbase() + DH_LUARESU));
...
void r_Arith(unsigned int rL, r_TValue* ra, r_TValue* rb, r_TValue* rc, r_TMS op)
{
	...
	_arith arith = (_arith)(danghui_rbase() + DH_LUARITH);
	...
	if (a2 <= 0)
	{
		if (a2 <= -10000)
		{
			switch (a2)
			{
				...
				case -10001:
					result = (DWORD *)(a1 + 72);
					*result = *(DWORD *)(**(DWORD **)(*(DWORD *)(a1 + 12) + 16) + 12);
					result[2] = 7;
					break;
				case -10000:
					result = (DWORD *)(a1 + 8 + *(DWORD *)(a1 + 8) + 168);
					break;
				default:
					v3 = **(DWORD **)(*(DWORD *)(a1 + 12) + 16);
					if (-10002 - a2 > *(BYTE *)(v3 + 7))
						result = NULL; // luaO_nilobject
					else
						result = (DWORD *)(v3 + 16 * (-10002 - a2) + 8);
					break;
			}
		}
		else
		{
			result = (DWORD *)(*(DWORD *)(a1 + 16) + 16 * a2);
		}
	}
	else
	{
		result = NULL; // luaO_nilobject
		if ((unsigned int)(16 * a2 + *(DWORD *)(a1 + 28) - 16) < *(DWORD *)(a1 + 16))
			result = (DWORD *)(16 * a2 + *(DWORD *)(a1 + 28) - 16);
	}
...
int danghui_wraplfn(lua_State* L, unsigned int rL, LClosure* fn)
{
	...
	void* cc = r_luaF_newCclosure(rL, 2, (void*)*(DWORD*)(rL + 104));
...
int danghui_indexud(lua_State* L, unsigned int rL, void* ud, TValue* key)
{
	r_TValue* indexresult = danghui_grabRT(L, rL); //new r_TValue;
	r_TValue* convertedkey = danghui_grabRT(L, rL); // new_rTValue;
...
int danghui_env_globalindex(lua_State *L)
{
	...
	r_TValue* env = cast(r_TValue*, (unsigned int*)(rL + 104));
...
void danghui_Aexecute(const char* script)
{
	...
	if (!luaL_loadstring(L, fscript.c_str()))
		lua_pcall(L, 0, 0, 0);
}